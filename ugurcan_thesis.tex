...
\committeememberi[assocprof]{TOLGA OVATMAN}
\affiliationi{Computer Engineering Department, ITU}
\committeememberii[assistprof]{TEOMAN NASKALI}
\affiliationii{Computer Engineering Department, GSU}
...
propriety hardware.  There is a significant amount of work in the network function virtualization domain
to actualize these network functions in a virtualized manner.  In this work we conducted a series of
performance tests to evaluate the compatibility of modern container based virtualization solutions for
NFV. To accomplish this, we implemented a generic service on top of a prevalent container orchestration
software Kubernetes and assessed its performance with numerous clients. Our conclusion is that they are
generally compatible except for some cases that may require certain specialized technologies to mature
for reliable use.}
...
teknolojisi olan işletim sistemi seviyesi sanallaştırma veya konteynır sanallaştırma ile uyumluluğunu
değerlendirebilmek için bir takım testler yaptık. Bunun için önde gelen konteynır yönetim sistemlerinden
Kubernetes üzerinde genel bir servis oluşturduk ve çeşitli miktarlarda istemciler kullanarak 
performanslarını inceledik. Vardığımız sonuç bu teknolojilerin genel anlamda birbirleriyle uyumlu olduğu
ancak bazı durumlar için gerekebilecek teknolojilerin olgunlaşması gerektiği yönünde oldu.
}
%
% Acknowledgements
...
Since mobile Internet usage is only expected to grow further, it's clear that telecom operators need
...
service companies to use a new and different way to design, develop, deploy and maintain
...
containers.  This approach is often called a cloud native approach.

% Tekrar daha detayli bir sekilde problemi acikla
...
We designed a few experiments for measuring end to end performance on Kubernetes using several different
setups.
...
allow more accuracy and predictable testing durations. For this experiment we will use another go based
open source load testing tool called vegeta for getting percentile metrics.Testing parameters for second
experiment can be found in Table \ref{secondexp}. CPU limits are expressed as a Kubernetes resource unit
...
In the Figure \ref{flannel} there are three applications application 1, 2 and 3. These applications are
consists of identical containers created by different deployments to measure the effect of co-location.
While applications 1 and 2 changes their instance counts on different tests application 3 stands as a
control application to see if unrelated containers gets affected from the system load.
