...
hardware rather than specialized devices.  But most of the network appliances in use are still runs on 
propriety
...
use these virtual network functions in a virtualization technology that is recently gaining traction called
...
litaratürde ağ işlevi sanallaştırma dalı altında pek çok çalışma bulunmaktadır.  Biz bu çalışmada ağ
...
relies on cloud computing recently started using a different way to design, develop, deploy and maintain
...
found in Table \ref{secondexp}. In Kubernetes 1000m refers to a single CPU core.
...
\hline
Setup & Deployment 1 & Deployment 2 & Deployment 3 \\ \hline
Setup A &  ~5 *  250m &  10 *  250m &  20 *  250m  \\ \hline
Setup B &  1 * 1250m &   1 * 2500m &   1 * 5000m  \\ \hline
\multirow{3}{*}{Setup C} 
 & (a) 10 *  125m & 20 *  125m & 40 *  125m \\
 & (b)~~5 *  250m & 10 *  250m & 20 *  250m \\
 & (c)~~3 *  500m &  ~~5 *  500m & 10 *  500m \\ \hline
\end{tabular}
\label{secondexp}
\end{table}
...
capable of running a higher number of containers.  High latency at the initial step is expected 
...
\begin{figure}
\centering
\includegraphics[]{ml_super_xl.png}
\caption{Setup B results for the second experiment}
\label{fig:ml_super_xl}
\end{figure}

\begin{figure}
\centering
\includegraphics[]{multi.png}
\caption{Setup C results for the second experiment}
\label{fig:multi}
\end{figure}

Without use of percentile values, there seems to be a significant difference between setup A and
setup B that suggests there is an huge overhead for using multiple containers. But percentile data
clearly shows for 95 percent of the connections performed very similarly in these cases.
But between 95th and 99th percentile deployments from setup A's performance decays more rapidly
compared to those in setup B whose performance only starts to decay in slowest 1 percent of the
connections.

For setup A instance counts of the deployments are doubled in each case and for setup B resource
limits of the deployments have been doubled.
For setup A there is a clear decrease in latency from deployment 1 to 2. But when instance count
increased again for deployment 3, performance subtlety worsened. Even thought
the increase in latency was barely noticeable compared those in setup B.
It is expected for instances in setup B's latency to decrease. It was assigned more resources while
load stayed the same. But decrease in latency was significant from deployment 1 to 2, It was minimal
for most connections from deployment 2 to 3 and only become noticeable in the slowest 5 percent of
the connections.
...
From setup C approach of experiments changes slightly. While comparing a single container
...
While latency decreased for all cases from deployment 1a, 1b, 1c to 2a, 2b and 2c, latency increased
or stayed similar from deployment 2a, 2b, 2c to 3a, 3b, 3c. This results from setup C also support
s the observations from the previous experiments that after a saturation point adding more instances
increases the overhead and therefore latency. This saturation point amounts to 2500m CPU resources
for these setup. 

The other important result from setup C is the significance of the balance between resource limits
and instance counts. All the deployments in setup C has an equal amount of total resources but there
is stark difference between their performance. With a correct configuration it's possible for multiple
containers to achieve similar performances to their single container counterparts. While deployment 1b
of setup C is seem to perform better than deployment 1 of setup B, it should be noted that the likely
cause is the extra 250m resources the former has. But for deployment 2b of setup C and deployment 2
of setup B. Albeit latter performs better, the overall performances is very similar.
...
a very small percent of results there is significant variance in latency. This may not acceptable for some
network functions. As a solution for this problme specialized high speed networking software may be needed.
Our conclusion was as is standard container clusters can not be used for implementing virtual network
functions without making use of the cloud native approach these systems were made to utilize. But since
